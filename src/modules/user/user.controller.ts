...
export const getUserMonsters = catchAsync(async (req: Request, res: Response) => {
  const userId = new mongoose.Types.ObjectId(req.params.userId);
  const options: IOptions = pick(req.query, ['sortBy', 'limit', 'page', 'projectBy']);
  const filter = { author: userId };
  const monsters = await monsterService.queryMonsters(filter, options);
  res.send(monsters);
});

export const getUsersGold = catchAsync(async (req: Request, res: Response) => {
  const result = await userService.getUsersGold();
  res.send(result);
});
